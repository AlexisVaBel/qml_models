...
Navigable::Navigable(QQuickItem *parent)
    : Super(parent), _containerItem(new QQuickItem{this}) {
  _containerItem->setTransformOrigin(TransformOrigin::TopLeft);
  _containerItem->setAcceptTouchEvents(true);

  connect(_containerItem,
          &QQuickItem::childrenRectChanged, // Listen to children rect changes
                                            // to update containerItem size
          [this]() {
  ...
  _defaultGrid = std::make_unique<Grid>();
...
void Navigable::centerOn(QQuickItem *item) {
  if (_containerItem == nullptr || item == nullptr)
    return;

  const qreal zoom = _containerItem->scale();

  // 1.
  const QPointF itemPos = _containerItem->mapToItem(
      item, QPointF{-item->width() / 2., -item->height() / 2.});
  ...
  const QPointF viewOriginContainerCs =
      mapToItem(_containerItem, QPointF{0, 0});
  const QPointF translationContainerCs =
      viewCenterContainerCs - viewOriginContainerCs;
  _containerItem->setPosition(_containerItem->position() +
                              (translationContainerCs * zoom));
}

void Navigable::centerOnPosition(QPointF position) {
  //
  QPointF navigableCenter{width() / 2., height() / 2.};
  QPointF navigableCenterContainerCs =
      mapToItem(_containerItem, navigableCenter);
...
void Navigable::fitInView() {
  ...
  if (!content.isEmpty()) { // Protect against div/0, can't fit if there is no
                            // content...
...
void Navigable::setZoom(qreal zoom) {
  if (isValidZoom(zoom)) {
    ...
    case QQuickItem::Center: {
      zoomOn(QPointF{width() / 2., height() / 2.}, zoom);
    } break;
...
void Navigable::zoomOn(QPointF center, qreal zoom) {
  ...
  // Don't apply modification if new zoom is not valid (probably because it is
  // not in zoomMin, zoomMax range)
  if (isValidZoom(zoom)) {
...
bool Navigable::isValidZoom(qreal zoom) const {
  ...
  if ((zoom > _zoomMin) && // Don't zoom less than zoomMin
      (_zoomMax <
           0. || // Don't zoom more than zoomMax except if zoomMax is infinite
       zoom < _zoomMax))
    return true;
  return false;
}
...
void Navigable::setAutoFitMode(AutoFitMode autoFitMode) {
  if (_autoFitMode != AutoFit && autoFitMode == AutoFit)
    fitInView(); // When going to auto fit mode from another mode, force
                 // fitInView()
...
void Navigable::geometryChanged(const QRectF &newGeometry,
                                const QRectF &oldGeometry)
#else
void Navigable::geometryChange(const QRectF &newGeometry,
                               const QRectF &oldGeometry)
#endif
{
  if (getNavigable()) {
    // Apply fitInView if auto fitting is set to true and the user has not
    // applyed a custom zoom or pan
    if (_autoFitMode == AutoFit && (!_panModified || !_zoomModified)) {
      fitInView();
    }
    // In AutoFit mode, try centering the content when the view is resized and
    // the content size is less than the view size (it is no fitting but
    // centering...)
    if (_autoFitMode == AutoFit) {
      ...
      QRectF contentBr =
          mapRectFromItem(_containerItem, _containerItem->childrenRect());
    ...
    // In AutoFit mode, try anchoring the content to the visible border when the
    // content has custom user zoom
    if (_autoFitMode == AutoFit && (_panModified || _zoomModified)) {
      ...
      } else if (anchorLeft) { // Right anchoring has priority over left
                               // anchoring...
...
void Navigable::mouseMoveEvent(QMouseEvent *event) {
  if (!getNavigable() || !_containerItem) {
  ...
  if (_leftButtonPressed && // Left click panning /////////////
    ...
    const auto p = QPointF{_containerItem->x(), _containerItem->y()} - delta;
    ...
    const auto selectionRect = mapRectToItem(
        _containerItem,
...
void Navigable::mousePressEvent(QMouseEvent *event) {
  ...
  if (event->button() ==
      Qt::RightButton) { // Right clicks are caught //////////
    event->accept();
    return;
  }
  event->ignore();
...
void Navigable::wheelEvent(QWheelEvent *event) {
  if (getNavigable()) {
    qreal zoomFactor =
        (event->angleDelta().y() > 0. ? _zoomIncrement : -_zoomIncrement);
    zoomOn(event->pos(), getZoom() + zoomFactor);
  }

  // Note 20160117: NavigableArea is opaque for wheel events, do not call
  // QQuickItem::wheelEvent(event);
}

void Navigable::setSelectionRectItem(QQuickItem *selectionRectItem) noexcept {
...
void Navigable::setGrid(Grid *grid) noexcept {
  if (grid != _grid) {
    if (_grid) { // Hide previous grid
      disconnect(_grid, nullptr, this,
                 nullptr); // Disconnect every update signals from grid to this
                           // navigable
    }
    ...
    if (_grid) {
      ...
      _grid->setScale(1.0);
      connect(
          grid, &QQuickItem::visibleChanged, // Force updateGrid when visibility
                                             // is changed to eventually
          this, &Navigable::updateGrid); // take into account any grid property
                                         // change while grid was hidden.
    }
    if (!_grid)
      _grid = _defaultGrid
                  .get(); // Do not connect default grid, it is an "empty grid"
    updateGrid();
    emit gridChanged();
  }
}
...
void Navigable::updateGrid() noexcept {
  if (_grid && _containerItem != nullptr) {
